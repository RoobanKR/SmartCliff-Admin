...
const YearlyServiceAddForm = () => {
  ...
  const [services, setServices] = useState([{ businessService: '', service: [''] }]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setSnackbar({ open: true, message: 'Yearly service created successfully!', severity: 'success' });
      ...
      setSnackbar({ open: true, message: 'Failed to create yearly service.', severity: 'error' });
    }
  };
          ...
          <form onSubmit={handleSubmit}>
            ...
            {services.map((service, index) => (
              <Grid container spacing={2} key={index}>
                ...
                <Grid item xs={6}>
                  <Button variant="outlined" onClick={() => handleAddSubService(index)} startIcon={<AddIcon />}>
                    Add Sub-Service
                  </Button>
                </Grid>
                {service.service.map((subService, subIndex) => (
                  <Grid item xs={12} key={subIndex}>
                    <Grid container spacing={2} alignItems="center">
                      ...
                      <Grid item xs={2}>
                        <IconButton color="error" onClick={() => handleRemoveSubService(index, subIndex)}>
                          <DeleteIcon />
                        </IconButton>
                      </Grid>
                    </Grid>
            ...
            <Button variant="outlined" onClick={handleAddService} startIcon={<AddIcon />}>
