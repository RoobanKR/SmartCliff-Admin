...
const HireFromUsFormsControl = () => {
  ...
  const hiringApplications =
    useSelector((state) => state.hireFromUs.hiringApplications) || [];
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        await dispatch(getAllHireFromUs());
        setLoading(false);
      } catch (error) {
  ...
  const handleDeleteConfirm = () => {
    dispatch(deleteHireFromUs(selectedApplicationId))
      .then(() => {
        dispatch(getAllHireFromUs());
  ...
  const handleSendEmailConfirm = async () => {
    ...
    try {
      ...
      setSnackbar({
        open: true,
        message: `Email sent successfully to ${response.results.success.length} hiringApplications`,
        severity: "success",
      });
    ...
    if (type === "applications") {
      ...
      const selectedData =
        selectedApplications.length > 0
          ? hiringApplications.filter((app) =>
              selectedApplications.includes(app._id)
            )
          : filteredApplications; // <-- This is the change: use filteredApplications instead of hiringApplications
      ...
      dataToExport = selectedData.map((app) => ({
        ...
        Email: app.email || "N/A",
        "Skillset Requirements": app.skillsetRequirements
          ? app.skillsetRequirements.map((skill) => skill.skillset).join(", ")
          : "N/A",
        Resources: app.skillsetRequirements
          ? app.skillsetRequirements.map((skill) => skill.resources).join(", ")
          : "N/A",
        Enquiry: app.enquiry || "N/A",
      }));

      fileName =
        "Hiring_Applications_Export_" +
  ...
  const filteredApplications = hiringApplications.filter((application) => {
    ...
    const skillsetMatch =
      ...
      (application.skillsetRequirements &&
        application.skillsetRequirements.some(
          (skill) =>
            skill.skillset &&
            skill.skillset.toLowerCase() === selectedSkillset.toLowerCase()
        ));
    ...
    if (appDateRangeFrom || appDateRangeTo) {
      ...
      const appDate = new Date(application.createdAt); // Change to your actual date field
    ...
    return (
      nameMatch && emailMatch && companyMatch && skillsetMatch && dateMatch
    );
  });
  ...
  const getRecipientNamesForDisplay = () => {
    if (tabValue === 0) {
      ...
      const selectedNames = hiringApplications
  ...
  const getAllSentEmails = () => {
    ...
    hiringApplications.forEach((hiring) => {
                    ...
                    <Grid container spacing={2} alignItems="center">
                      ...
                      <Grid item xs={12} sm={6} md={3}>
                        <FormControl fullWidth size="small">
                          <InputLabel id="skillset-filter-label">
                            Skillset
                          </InputLabel>
                          <Select
                        ...
                        <Grid container spacing={1}>
                          ...
                          <Grid item xs={6}>
                            <TextField
                              ...
                              InputLabelProps={{ shrink: true }}
                              onChange={(e) =>
                                setAppDateRangeTo(e.target.value)
                              }
                    ...
                    <Grid container spacing={2} alignItems="center">
                      ...
                      <Grid item xs={12} sm={6}>
                        <TextField
                          ...
                          onChange={(e) =>
                            setEmailNameSearchTerm(e.target.value)
                          }
                          ...
                          >
                            ...
                            >
                              <Checkbox
                                ...
                                onChange={() =>
                                  handleCheckboxChange(application._id)
                                }
                              />
                            </TableCell>
          ...
          >
            ...
            <DialogContent sx={{ py: 3 }}>
              <Typography variant="body1">
                Are you sure you want to delete this hiring application? This
...
export default HireFromUsFormsControl;
