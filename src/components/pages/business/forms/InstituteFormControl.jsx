  ...
  const [snackbar, setSnackbar] = useState({
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                await dispatch(getAllInstitute());
                setLoading(false);
            } catch (error) {
    ...
    const handleDeleteConfirm = () => {
        dispatch(deleteInstitute(selectedApplicationId))
            .then(() => {
    ...
    const handleSendEmailConfirm = async () => {
       if (!emailSubject || !emailMessage) {
          ...
          try {
            const response = await dispatch(sendResponseEmailInstituteForm({
            ...
            setSnackbar({
              open: true,
              message: `Email sent successfully to ${response.results.success.length} instituteApplications`,
              severity: "success"
            });
    ...
    const exportToExcel = (type) => {
        ...
        if (type === 'applications') {
            ...
            const selectedData = selectedApplications.length > 0
                ? instituteApplications.filter(app => selectedApplications.includes(app._id))
                : filteredApplications; // <-- This is the change: use filteredApplications instead of instituteApplications
 
            dataToExport = selectedData.map(app => ({
                'Name': app.name || 'N/A',
                'Company Name': app.institute_name || 'N/A',
                'Mobile': app.mobile || 'N/A',
                'Email': app.email || 'N/A',
                'Skillset Requirements': app.services ? app.services.map(skill => skill.service).join(', ') : 'N/A',
                'Resources': app.services ? app.services.map(skill => skill.resources).join(', ') : 'N/A',
    ...
    const filteredApplications = instituteApplications.filter(application => {
        ...
        const companyMatch = application.institute_name ?
            application.institute_name.toLowerCase().includes(companySearchTerm.toLowerCase()) :
            companySearchTerm === '';

        const skillsetMatch = selectedSkillset === '' ||
            (application.services &&
                application.services.some(skill =>
                    skill.service && skill.service.toLowerCase() === selectedSkillset.toLowerCase()
                ));
    ...
    const getRecipientNamesForDisplay = () => {
        if (tabValue === 0) {
            ...
            const selectedNames = instituteApplications
    ...
    const handleCloseSnackbar = () => {
      ...
      const getAllSentEmails = () => {
        ...
        instituteApplications.forEach(hiring => {
          ...
          >
            Back
                    ...
                    <Box sx={{ mb: 4 }}>
                        ...
                        >
                            Institute Applications Records
                        </Typography>
                                        ...
                                        <Grid container spacing={2} alignItems="center">
                                            ...
                                            <Grid item xs={12} sm={6} md={3}>
                                                <FormControl fullWidth size="small">
                                                    <InputLabel id="service-filter-label">Skillset</InputLabel>
                                                    <Select
                                                    ...
                                                    >
                                                        ...
                                                        {uniqueSkillsets.map((service, index) => (
                                        ...
                                        {filteredApplications.length > 0 ? filteredApplications
                                            ...
                                            .map((application) => (
                                                ...
                                                >
                                                    ...
                                                    <TableCell>
                                                        <Typography fontWeight={500}>
                                                            {application.institute_name || 'N/A'}
                                                        </Typography>
                                                    </TableCell>
                                                    <TableCell>
                                                        <Typography fontWeight={500}>
                                                    ...
                                                    <TableCell>
                                                        <Typography fontWeight={500}>
                                                            {application.services.map(skill => skill.service).join(', ') || 'N/A'}
                                                        </Typography>
                                                    </TableCell>
                                                    <TableCell>
                                                        <Typography fontWeight={500}>
                                                            {application.services.map(skill => skill.resources).join(', ') || 'N/A'}
                                                        </Typography>
                                                    </TableCell>
                                                    <TableCell>
                                                        <Typography fontWeight={500}>
...
export default InstituteUsFormsControl;